...
void *threadLobby(void *arg)
{
    ...
    while (1)
    { // Ciclo per assicurarsi che il nome sia valido
    ...
    while (1)
    {
        ...
        if (strcmp(buffer, MSG_CLIENT_CREAATE) == 0)
        {
            ...
            partita->giocatoreAdmin.stato = 0;
            ...
            while (1)
            {
                ...
                if (giocatore->socket == lobby.partita[nuovoId].Vincitore)
                {
                    ...
                    if (strcmp(buffer, MSG_CLIENT_REMATCH) == 0)
                    {                                     // il giocatore ha scelto di fare un rematch
        ...
        { // il giocatore ha scelto di unirsi a una partita
            ...
            { // altrimenti invio la lista delle partite disponibili
            ...
            if (lobby.partita[partitaScelta].statoPartita == PARTITA_IN_ATTESA)
            {
                ...
                if (strcmp(buffer, MSG_CLIENT_ACCEPT) == 0)
                {
                    ...
                    if (pthread_detach(thread) != 0)
                    { // DETACH DEL THREAD PARTITA
                        perror("[Lobby] Errore nel detach del thread per la partita\n");
                        break;
                    }
                }
                else if (strcmp(buffer, MSG_CLIENT_REFUSE) == 0)
                { // il giocatore ha rifiutato la richiesta di join
            ...
            { // la partita non è disponibile (es. qualcuno si è unito prima)
                pthread_mutex_lock(&lobby.lobbyMutex);
        ...
        else if (strcmp(buffer, MSG_CLIENT_QUIT) == 0)
        { // il giocatore ha scelto di uscire dalla lobby e quindi dal server
            break;
        }
        else
        {
...
void *threadPartita(void *arg)
{
    ...
    while (1)
    {
        ...
        while (1)
        {
            ...
            if (eseguiMossa(partita->Griglia, coordinate[0], coordinate[1], simboloGiocatoreCorrente) == 0)
        ...
        if (check_winner(simboloGiocatoreCorrente, partita))
        { // se il giocatore corrente ha vinto
        ...
        if (contatoreTurno == 8)
        {
            if (is_draw(partita))
            {
                ...
                if (strcmp(buffer, MSG_CLIENT_QUIT) == 0)
                { // il proprietario ha scelto di uscire tornano entrambi al menu
                ...
                else if (strcmp(buffer, MSG_CLIENT_REMATCH) == 0)
                { // il proprietario ha scelto di fare un rematch quindi informo il guest
                    ...
                    { // il guest ha scelto di fare un rematch quindi ricomincia la partita corrente
                        sprintf(buffer, MSG_SERVER_REMATCH);
                        if (send(partita->giocatoreAdmin.socket, buffer, strlen(buffer), 0) < 0)
                        {
                            perror("[Partita] Errore nell'invio del messaggio di abbandono del guest\n");
